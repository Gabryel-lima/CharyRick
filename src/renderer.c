...
bool renderer_init(Renderer *renderer, SDL_Renderer *sdl_renderer) {
    ...
    int max_y = 0;
    int advance = 0;
    if (TTF_GlyphMetrics(renderer->font, 'M', &min_x, &max_x, &min_y, &max_y, &advance) == 0 && advance > 0) {
        renderer->cell_w = advance;
    }

    if (renderer->cell_w <= 0) {
        int text_w = 0;
        int text_h = 0;
        if (TTF_SizeText(renderer->font, "M", &text_w, &text_h) == 0 && text_w > 0) {
            renderer->cell_w = text_w;
        }
    }
